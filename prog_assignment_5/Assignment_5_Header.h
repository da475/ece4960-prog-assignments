...
#define THREEFOUR 7
#define BISECTION 8
#define CONSTANT_E_H 9
#define USE_O_H 10
#define EXCEPTION_HANDLING false    // Macro to enable exception handling
#define PRINT_VALUES false    // Print values
#define WRITE_VALUES true    // Write values to csv file
...
extern double tolerance1;
extern double tolerance2;
extern double tolerance3;
extern double tolerance4;
...
public:
    ...
    static void write_Comparison(fullVect *time, arrayMat *calValues, string name, double step, double order, arrayMat *trueValues = NULL) 
    {
        if ((time->dim != calValues->dim) || (trueValues != NULL && (time->dim != trueValues->dim))) throw UNMATCHED_DIM;
	...
	ofstream outdata;
        ...
        if (trueValues != NULL)
        {
            string fileName = "/Users/Vishisht/Desktop/HackerPractice/prog_assignment_5/Results/" + name + ".csv";
            const char *fileChar = fileName.c_str();
            remove(fileChar);
            outdata.open(fileChar, ios::app);
            outdata << "Time,True Values,Calculated Values,Calculated Error(%)" << endl;
            for (int i = 0; i < dim; i++)
            {
                outdata << to_string(time->arr[i]) + "," + 
                        to_string(trueValues->arr[i]->col[0]) + "," + 
                        to_string(calValues->arr[i]->col[0]) + "," + 
                        to_string(calculate_Error(trueValues->arr[i]->col[0],calValues->arr[i]->col[0])) + ","  << 
                        endl;
                #if EXCEPTION_HANDLING
                    if (isinf(time->arr[i])) throw IS_INF_NINF;
                    if (isnan(time->arr[i])) throw IS_NAN;
            ...
            string fileName = "/Users/Vishisht/Desktop/HackerPractice/prog_assignment_5/Results/" + name + "_step(ns)_" + to_string(step * 1e9) + "_order_" + to_string(order) + ".csv";
            const char *fileChar = fileName.c_str();
            remove(fileChar);
            outdata.open(fileChar, ios::app);
            outdata << "Time(ns),Calculated Values(V1),Calculated Values(V2)" << endl;
            for (int i = 0; i < dim; i++)
            {
                outdata << to_string(time->arr[i] * 1e9) + "," + 
                        to_string(calValues->arr[i]->col[0]) + "," + 
                        to_string(calValues->arr[i]->col[1]) + ","  << 
                        endl;
                #if EXCEPTION_HANDLING
                    if (isinf(time->arr[i])) throw IS_INF_NINF;
                    if (isnan(time->arr[i])) throw IS_NAN;
